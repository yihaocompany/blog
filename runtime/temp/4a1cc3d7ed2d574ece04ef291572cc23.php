<?php /*a:2:{s:58:"D:\wwwroot\blog.com\application\admin\view\public_head.php";i:1550285504;s:62:"D:\wwwroot\blog.com\application\admin\view\background_edit.php";i:1550288349;}*/ ?>
...
<div class="aright">
    ...
    <form class="layui-form bform" method="post" enctype="multipart/form-data">
        ...
        <div class="layui-form-item">
            <label class="layui-form-label">侧图</label>
            <div class="layui-input-block">
                <div class="file-box">
                    <i class="layui-icon">&#xe61f;</i>
                    <input class="file-btn" type="button" value="选择图片">
                    <input class="file-txt" type="text" autocomplete="off"  id="textfield3">
                    <?php if(($info['side_img'])): ?><img src="<?php echo htmlentities((isset($info['side_img']) && ($info['side_img'] !== '')?$info['side_img']:'')); ?>" width="120"><?php else: ?><?php endif; ?>
                    <input class="file-file" type="file" name="side_img" id="pic" size="28" onchange="document.getElementById('textfield3').value = this.value" />
                </div>
            </div>
        </div>
        <div class="layui-form-item" style="width: 300px;">
            <label class="layui-form-label">显示侧图</label>
            <div class="layui-input-block">
                <select name="status">
                    <?php if(is_array($notes['is_side']) || $notes['is_side'] instanceof \think\Collection || $notes['is_side'] instanceof \think\Paginator): $i = 0; $__LIST__ = $notes['is_side'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                        <option <?php if(($info['is_side']==$key)): ?>selected="selected"<?php endif; ?> value="<?php echo htmlentities($key); ?>"><?php echo htmlentities($vo); ?></option>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
