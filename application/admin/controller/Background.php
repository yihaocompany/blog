...
class Background extends AdminBaseController {
    ...
    public function edit() {
        ...
        if (IS_POST) { //数据操作
            ...
            if (!empty($_FILES['side_img']['tmp_name'])) {
                $file3 = request()->file('side_img'); //图片上传
            }
            ...
            if ($file3) {
                $file_path3 = \think\facade\Env::get('ROOT_PATH') . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads';
                $img_path3 = DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR;
                $img_info3 = $file3->move($file_path3);
                if ($img_info3) {
                    $data['side_img'] = $img_path3 . $img_info3->getSaveName();
                }
            }
